...
def resolution(formulas, max_iterations=10):
...
"""formulas = [
    #A -> (B -> A)
    Expression(Implication(A,Implication(B, A))),
    #((A -> (B -> C)) -> ((A->B) -> (A->C)))
    Expression(Implication(Implication(A,Implication(B, C)), Implication(Implication(A, B), Implication(A, C)))),
    #((not B -> not A) -> ((not B -> A) -> B))
    Expression(Implication(Implication(Negation(B), Negation(A)), Implication(Implication(Negation(B), A), B)))
]"""
A = Variable("A")
B = Variable("B")
C = Variable("C")
...
formulas = [
    Expression(Implication(A, B)),
    Expression(Implication(B, C)),
    Expression(Implication(C, D)),
    Expression(Implication(D, E)),
    Expression(Implication(E, Negation(A)))  # Противоречивая посылка
]
for f in formulas:
    f_new = Expression(Implication(Negation(f.content.consequent), Negation(f.content.antecedent)))
    formulas.append(f_new)
print(formulas)
"""
# Проверяем на противоречие
